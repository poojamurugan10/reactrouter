...
const ProductItem = ({ product, inCart, addToCart, removeFromCart }) => {
  return (
    <div className="bg-white  p-4 shadow-2xl rounded-lg">
      ...
      <p className="text-md font-bold text-black  p-2 ml-2 flex gap-1">
        <FaDollarSign />
        {product.price}
      </p>
      <h3 className="text-md font-medium text-gray-600  p-2 ml-2">
        {product.category}
      </h3>
      <span className="flex gap-35">
        <p className="text-md font-bold text-black  ml-3 flex gap-1">
          <IoStarOutline />
          {product.rating.rate}
        </p>
        <p className="text-md font-bold text-black  ml-3 flex gap-1">
